...
const fieldstatus = Object.entries(fieldStatus).map(([key, value]) => ({
 ...
 const FormCreate = (
          ...
          <>
            <div key="approvedBy" className="field" style={{ marginTop: "1.6rem" }}>
              <span className="contentfloat" style={{ width: "100%" }}>
                ...
                <label htmlFor="content" >{lang === "LA" ? "ເນື້ອໃນ ການລວງເວລາ" : "Content"} <span className='required-star' >*</span></label>
              </span>
            </div>
            <div className="grid">
              <div className="col-6">
  ...
  const header = (<div style={{ width: "100%", display: "flex", justifyContent: "center", color: "#2684FF" }}><div>ການລວງເວລາ ID: ({rowItem?.ot_id})</div></div>)
