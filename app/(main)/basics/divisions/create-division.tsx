...
import { CreateDivisionInput, createDivisionSchema } from '@/utils/validators/create-division.schema';
import { PiBuildingOfficeFill } from "react-icons/pi";

export default function CreateDivision(defaultdata: any) {
  const [lang, setLang] = useState("LA");
  const [reset, setReset] = useState({});
  const [openModal, setopenModal] = useState(false)
  const handOpen = () => { setopenModal(true) }
  const handClose = () => { setopenModal(!openModal) }
  const onSubmit: SubmitHandler<CreateDivisionInput> = async (data) => {
    try {
      const formattedData = {
        ...data,
      };
  ...
  const FormCreate = (
    <Form<CreateDivisionInput> id="createExportForm" resetValues={reset} validationSchema={createDivisionSchema} onSubmit={onSubmit} className="p-fluid" >
          ...
          <>
            <div key="ex_code" className="field" style={{ marginTop: "1rem" }}>
              <span className="contentfloat" style={{ width: "100%" }} >
                ...
                <label htmlFor="ex_code" >ຊື່ ຝ່າຍ/ຫ້ອງການ/ສະຖາບັນ <span className='required-star' >*</span></label>
              </span>
