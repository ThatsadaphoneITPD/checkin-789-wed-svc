...
interface CreateSickLeaveProps {
    rowItem: Checkin.SickLeave;
}
      ...
      approveSickLeave(formData).then((res: any)=>{ console.log("res", res) 
        if(res?.status == 201) {
          if (res.approvething == "Approved") {
            ...
            handClose();
            toast(`ໄດ້ປະຕິເສດ ການຄອບພັກ ເລກທີ ${rowItem?.leave_req_id}`, { icon: <RocketFlamingIcon style={{width: "1.5rem", height: "1.5rem"}}/>, style: { border: '1px solid #FFA500', color: '#333', background: '#FFFAE5',  }, duration: 5000,} );
          }
          ...
          <>
            <div key="approvedBy" className="field" style={{ marginTop: "1.6rem" }}>
              <span className="contentfloat" style={{ width: "100%" }}>
                <InputTextarea  defaultValue={rowItem?.reasons} disabled rows={2} cols={20} />
                <label htmlFor="content" >{lang === "LA" ? "ເນື້ອໃນ ການລາພັກ" : "Content"} <span className='required-star' >*</span></label>
              </span>
            </div>
