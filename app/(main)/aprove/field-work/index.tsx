...
export default function FieldWorkTable() {
    ...
    const filterData = (filter: string) => {
        if (!filter.trim()) {
            ...
            const filtered = data?.filter((item: any) =>
                item?.fw_req_id?.toString().toLowerCase().includes(lowercasedFilter) ||
    ...
    return (
        <div>
            {header}
            <DataTable dataKey="fw_req_id"
                sortField="fw_req_id" sortOrder={1} 
                rows={10} paginator ref={dt}
                value={filteredData?.map((item, index) => ({ ...item, _key: `${item?.fw_req_id ?? 'row'}-${index}` }))}
